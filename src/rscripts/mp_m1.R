...
data_c <- read.csv(paste(src_p, "_counter.csv", sep = ''))

model.y1 <- lm(Y ~ G * R * ns(X, df = 4, intercept = TRUE) - 1, data = data_i)
write.csv(data.frame(summary(model.y1)$coefficients), file=paste(counter_f, "R1_y.csv", sep=''))

y_counter <- resid(model.y1) + predict(model.y1,  data_c)
write.csv(data.frame(y_counter), file=paste(counter_f, "R1_y_counter.csv", sep=''))
